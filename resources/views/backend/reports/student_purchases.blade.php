                            ...
                            @if (isset($purchases) && count($purchases) > 0)
                                @foreach ($purchases as $purchase)
                                    <tr>
                                        ...
                                        <td>{{ (isset($purchase->transaction_id)?$purchase->transaction_id:'--') }}</td>
                                        <td>{{ (isset($purchase->student->student_name)?$purchase->student->student_name:'--') }}</td>
                                        <td>{{ (isset($purchase->product->package_name)?$purchase->product->package_name:'--') }}</td>
                                        <td>{{ (isset($purchase->amount)?$purchase->amount:'--') }}</td>
                                        <td>{{ (isset($purchase->status)?ucfirst($purchase->status):'--') }}</td>
                                        <td>{{ (isset($purchase->created_at)?date('d/m/Y h:i:s', strtotime($purchase->created_at)):'--') }}</td>
